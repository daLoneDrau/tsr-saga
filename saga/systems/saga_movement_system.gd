...
#
# Scope note: this handles HERO movement only. Jarl movement (aligned jarls
# moving in the same turn-order slot as their hero, using either the hero's
# or their own MOVEMENT_SPEED depending on whether they move together) is
# out of scope here and will be added alongside the combat/recruitment phase.
#
...
# state). Location state itself is owned by SagaBoardSystem — this system
# never writes to it directly, only through SagaBoardSystem's public API.
#
# Listens for:
...
## SagaTurnSystem and resets the mover cursor to the first hero.
...
## Advances the mover cursor on success. Returns false (no state changed)
## if it isn't this hero's turn or the destination is illegal.
